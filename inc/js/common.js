...
var size=$('.firstdiv table tr td').size();
var counter=0;


$('#down').click(function(e){
    e.preventDefault();  //remove default behaviour of a link 

    if(counter<size-1){
        var animationSpeed=1200;
    // $('table').css('margin-left', '-200px');
    $('table').animate({marginLeft:"-="+1070+"px"}, animationSpeed);
    counter++;
    }
...
// function contact_form_submit(){
    jQuery('button#contact-us-button').click(function() {
        ...
        jQuery.ajax({
          type: "POST",
          url: "sendEmail.php",
          data: { your_name: name, email: email, message: message }
        })
          .done(function( msg ) {
            if(msg == 'sent') {
...
// function contact_form_submit(){
    jQuery('#feedback-button').click(function() {
        ...
        jQuery.ajax({
          type: "POST",
          url: "feedback.php",
          data: { fname: name, Country: country, ycomment: message }
        })
          .done(function( msg ) {
            if(msg == 'success') {
                jQuery('span.feedback-message-sent').text('Feedback Submitted');

                setTimeout(hideFeedbackMessage,5000);
            } else {
                jQuery('span.feedback-message-sent').text('Couldn\'t submit feedback');
                setTimeout(hideFeedbackMessage,5000);
            }
          });
    ...
    function hideFeedbackMessage(){
        jQuery('span.feedback-message-sent').fadeOut();
    }


    function hideMessage(){
        jQuery('.message-sent').fadeOut();
    }
//  }
...
=============================================== */
    ...
    var iconBase = 'img/dd-end.png';
    var myLatlng = new google.maps.LatLng(27.679804, 85.315236);
